...
int main()
{
	...
	char* fp = &file;
	Position hash_table[SIZE_HASH] = { NULL };
	int i = 0;
	
	for (i = 0; i < SIZE_HASH; i++) {
		Position head = NULL;
		head = CreateNewCountry("");
		hash_table[i] = head;
	}

	if (OpenFile(fp, &hash_table) == EXIT_FAILURE)
		return EXIT_FAILURE;

	for (i = 0; i < SIZE_HASH; i++) {
		printf("COUNTRIES WITH KEY %d:\n", i);
		PrintList(hash_table[i]);
	}

	UserInput(&hash_table);

	return EXIT_SUCCESS;
}

int OpenFile(char* file, Position* hash)
{
	FILE* fp = NULL;
	fp = fopen(file, "r");
	if (fp == NULL) {
	...
	else if (ReadFromFile(file, hash) == EXIT_FAILURE) {
		fclose(fp);
		return EXIT_FAILURE;
	}
...
int ReadFromFile(char* filename, Position* hash)
{
	char buffer[MAX] = { 0 };
	...
	while (!feof(file)) {
		...
		if ((int)*last_char >= 65 && (int)*last_char <= 122) { //zadnji znak je slovo
			...
			if (sscanf(p_buffer, "%s %s", temp_name, file_name) == 2) {
				...
				key = CalculateKey(name);
				AddNewSorted(new_country, *(hash + key));
			...
			else (*hash)->cities_tree = AddNewCity(new_city, (*hash)->cities_tree);
		}
	}
	fclose(file);
	return EXIT_SUCCESS;
...
int AddNewSorted(Position New, Position p)
{
